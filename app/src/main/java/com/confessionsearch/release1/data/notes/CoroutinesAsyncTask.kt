...
import com.confessionsearch.release1.data.notes.StatusState.Status
...
abstract class CoroutinesAsyncTask<Params, Progress, Result>(taskNames: String)
{
  ...
  var status: StatusState.Status = StatusState.Status.PENDING
  var preJob: Job? = null
  ...
  fun cancel(mayInterruptIfRunning: Boolean)
  {
    ...
    if (mayInterruptIfRunning || (!preJob!!.isActive && !bgJob!!.isActive))
    {
      ...
      preJob?.cancel(
        CancellationException("PreExecute: Coroutine Task cancelled")
      )
      bgJob?.cancel(
        CancellationException("doInBackground: Coroutine Task cancelled")
      )
